...
bool SaveData (int numberTracks, int offset, float** trackFloatData, unsigned char** trackUcharData, int** trackIntData, char** buffer);
...
void MapTrackOffset(int trackOffset, NPnodePtr node, char* propertyName, void* dataRef)
{
	...
	if (strcmp(propertyName, "rotate_rate_x") == 0) MapTrackToFloatAttribute (trackOffset, &node->rotateRate.x, data);
	if (strcmp(propertyName, "rotate_rate_y") == 0) MapTrackToFloatAttribute (trackOffset, &node->rotateRate.y, data);
	if (strcmp(propertyName, "rotate_rate_z") == 0) MapTrackToFloatAttribute (trackOffset, &node->rotateRate.z, data);
	if (strcmp(propertyName, "scale_rate_x") == 0) MapTrackToFloatAttribute (trackOffset, &node->scaleRate.x, data);
...
//------------------------------------------------------------------------------
bool SaveData(int numberTracks, int offset, float** trackData, unsigned char** trackUcharData, int** trackIntData, char** buffer)
{
	int i = 0;
...
//------------------------------------------------------------------------------
void loadTracksIntoCircularBuffer (void* dataRef)
{
	pData data = (pData)dataRef;
	pNPch ch = &data->io.ch;
	FILE* fp = NULL;

	fp = ch->fileDataSource;
	...
	while ( fp != NULL )
	{
		while (SaveData (ch->numberTracks, ch->channelWriteIndex++,
			ch->trackFloatData, ch->trackUcharData, ch->trackIntData, npChTrackFileTokens(fp, data)) )
		{
		...
		npFileClose (fp, dataRef);
		fp = ch->fileDataSource = NULL;

		// if we are looping, and the buffer cycled, re-read the file
		if ( loopAtEndOfTrack 
			&& ch->channelDataSize == kNPtrackBufferSize
			&& nposSupportsAntzThreads() ) 
		{
			data->io.ch.channelReadIndex = kNPtrackBufferSize - 1;
			data->io.ch.channelWriteIndex = 0;
			fp = ch->fileDataSource = npFileOpen (ch->fullFilePath, "r", data);
		}
	}
	nposEndThread();
}
...
void npLoadChMap (const char* filePath, void* dataRef)
{
	...
	if (file != NULL) 
	{
		tokens = tokenScanner = npChFileTokens (file, data);
		...
		// first line was header, now start with data
		tokens = npChFileTokens(file, data);
		while (tokens != NULL) 
		{
			data->io.ch.xref[data->io.ch.xrefIndex].attributeName = 
					npNewStrcpy(*(tokens + kNPattributeNameOffset), data);
			data->io.ch.xref[data->io.ch.xrefIndex].trackOffset = 
					atoi(*(tokens + kNPtrackOffset));
...
void npLoadChTracks (const char* filePath, void* dataRef)
{
	...
	if (file != NULL)
	{
		ch->fullFilePath = npNewStrcpy (filePath, data);				//zz-JJ
		ch->fileDataSource = file;
		ch->numberTracks = SaveHeaders ( npChFileTokens(file, data), data);
		...
		nposBeginThread (loadTracksIntoCircularBuffer, data);
		npPostMsg ("Done Loading Tracks", kNPmsgCtrl, data);

	} 
	else
	{
		sprintf (msg, "Failed opening %s\n", filePath);
		npPostMsg (msg, kNPmsgErr, data);
	}
}

void* npChReadLine(char* bufferStart, int maxLength, FILE* file)
{
	int cval = 0;
	int i = 0;
...
// ---------------------------------------------------------------------------------------
char** npChFileTokens (FILE* file, void* dataRef)
{
	pData data = (pData) dataRef;
	pNPch ch = &data->io.ch;
	...
	if (npChReadLine(&ch->channelFileTokenBuffer[0], kNPmaxLineLength, file) == NULL)
		return NULL;
...
char** npChTrackFileTokens (FILE* file, void* dataRef)
{
	pData data = (pData) dataRef;
	pNPch ch = &data->io.ch;
	...
	//read a chunk of the file
	if (npChReadLine(&ch->trackFileTokenBuffer[0], kNPmaxLineLength, file) == NULL)
		return NULL;
...
void npChSubscribeNode (NPnodePtr node, void* dataRef)
{
	...
	for (i = 0; i < ch->xrefIndex; i++) {
		if (node->chInputID == ch->xref[i].channelNumber) {
			sprintf(msg, "node %d uses channel %d, mapping attribute %s", node->id, node->chInputID, ch->xref[i].attributeName);
			npPostMsg(msg, kNPmsgDebug, dataRef);
...
//zz-JJ		printf("Reading %d, channelWriteIndex %d, channelDataSize %d\n", data->io.ch.channelReadIndex, data->io.ch.channelWriteIndex, data->io.ch.channelDataSize);
		}
	}
