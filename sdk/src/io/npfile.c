...
void npOpenURL (const char* command, void* dataRef)							//zz html replace entire function
{
	...
	char sysCmd[kNPurlMax] = {"open \0"};	//generally 2048 is max URL length
     ...
     */
     //zz html comment out this - end
			sprintf( sysCmd, "open %s%d", data->io.url, node->recordID );	//zz html leave this
	}
